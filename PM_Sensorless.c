...
//    Uint32 toplamGonderilecekData = 629136; // DAC için gönderilecek 24 bitlit data
	Uint16 zaman = 1;
	int ix = 0;
	int iy = 100;     // adc için for döngüsü
	Uint32 adcOrtDeger = 0;     // adc nin bit olarak ortalama degeri
	float verilenVolt = 0;     // dac a verilecek milivolt ( volt cinsinden )
	float adcVoltDeger = 0;     // adc ortalama degerin volt cinsinden karsiligi
	float bolumDeger = 0;     // verilen milivolt ile adc ref voltajin bolumu
	Uint32 dacData = 0;     // dac a yazilacak deger
	float temp1=0.0;    // dac a yazilacak derece ( C cinsinden )
	float temp2=0.0;
	float temp3=0.0;
	float offSet = -979.68;//-1004.35;     // -997.5;     //-1002.56;//2.352;   // of
	Uint16 b = 0;
	Uint16 a = 0;
	Uint16 c = 16;

	int Sicaklik = 0;
	float offsetDeger = 1.15;
...
// Initialize SPI for communication to the DRV8301
	...
	SPI_Freq_Init( &SpibRegs );
  //SPI_Baslat( &SpiaRegs ); /*Programin bu kismi tamamlaninca aktif edilecek*/
#endif
...
//-----------------------------------------------------------------
{
	...
	if( zaman == 0 )
	{
		/*Temperature for TEMP1*/
		GpioDataRegs.GPACLEAR.bit.GPIO3 = 1; // SYNC pin of DAC sets to clear
		DELAY_US(30000);
		if( GpioDataRegs.GPADAT.bit.GPIO3 == 0 )
		{
			AD5680_TEMP_SEND( temp1+ calTemp1, offSet );
			DELAY_US(30000);
			GpioDataRegs.GPASET.bit.GPIO3 = 1; // SYNC pin of DAC sets to logic 1 after complete the data transfer
		}

		/*Temperature for TEMP2*/
		GpioDataRegs.GPACLEAR.bit.GPIO5 = 1; // SYNC pin of DAC sets to clear
		DELAY_US(30000);
		if( GpioDataRegs.GPADAT.bit.GPIO5 == 0 )
		{
			AD5680_TEMP_SEND( temp2 + calTemp2, offSet );
			DELAY_US(30000);
			GpioDataRegs.GPASET.bit.GPIO5 = 1; // SYNC pin of DAC sets to logic 1 after complete the data transfer
		}

		/*Temperature for TEMP3*/
		GpioDataRegs.GPACLEAR.bit.GPIO7 = 1; // SYNC pin of DAC sets to clear
		DELAY_US(30000);
		if( GpioDataRegs.GPADAT.bit.GPIO7 == 0 )
		{
			AD5680_TEMP_SEND( temp3 +calTemp3, offSet );
			DELAY_US(30000);
			GpioDataRegs.GPASET.bit.GPIO7 = 1; // SYNC pin of DAC sets to logic 1 after complete the data transfer
		}
...
//-----------------------------------------
{
		...
		if( zaman_f == 0 )
		{
			//fOut = fOut * ( AD9833Register / AD9833_Master_Clock );
			fAD9833 = (Uint32) ( fOut * 11.18 );
			AD9833_Freq_Send( fAD9833, waveTypeF );
			zaman_f++;
		}
	//-----------------
...
//----------------------------------------
{
		...
		if( buttonSay > 9 )
		{
			if( GpioDataRegs.GPADAT.bit.GPIO0 == 0 )     // Inc(+) button control
			{
				if (cikisType==0)fOut += 100;
				else if (cikisType==1)temp1+=10;
				else if (cikisType==2)temp2+=10;
				else if (cikisType==3)temp3+=10;
			}
			else if( GpioDataRegs.GPADAT.bit.GPIO2 == 0 )     // Dec(-) button control
			{
				if (cikisType==0)
				{
					fOut -= 100;
			...
			else if( GpioDataRegs.GPADAT.bit.GPIO4 == 0 )     // Select button control
			{
				uzunBasiliDurum = 3;
				return;
			}
			else if( GpioDataRegs.GPADAT.bit.GPIO30 == 0  )     // Ok button control
			{
				uzunBasiliDurum = 4;
				return;
			}

		/*Check for Ok button on hold button state*/
		if( uzunBasiliDurum == 4 & buttonSay > 100 ) {zaman_f = 0; zaman=0;}
		else if( uzunBasiliDurum == 4 & buttonSay < 100 ) {waveTypeF++; waveTypeF %= 3;}

		else if( uzunBasiliDurum == 3 & buttonSay < 100 ){}
		else if( uzunBasiliDurum == 3 & buttonSay > 100 ){cikisType++; cikisType %= 4;}
